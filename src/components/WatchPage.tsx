...
// WatchPage is a fake video player screen, which demonstrates the movie watching view and the end prompt to post about the movie
// It uses the selected movie image as the background instead of a real movie
// This helps us display the UI and demo and end-toend task for a user without having to store real movies
...
export function WatchPage({ movie, onBack, onPostAboutMovie }: WatchPageProps) {
  ...
  // Reset whenever new movie is opened
  useEffect(() => {
    setProgress(0);
    setIsPlaying(true);
  ...
  // This simulates the movie playing, Small update every 100ms to simulate smooth progress
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      setProgress((current) => {
        ...
        // When bar reaches the end, stop and show the post prompt
        if (next >= 100) {
  ...
  // Dragging the progress bar calls this
  ...
  // Return the JSX that renders the video player
  return (
    <div className="min-h-screen bg-black text-white">
      <div className="relative min-h-screen overflow-hidden">
        ...
        <main className="relative z-10 flex min-h-[calc(100vh-88px)] flex-col justify-end px-6 pb-8 sm:px-8">
          ...
          {showEndPrompt && (
            // This appears when the progress reaches 100%.
          ...
          <div className="rounded-lg bg-black/55 p-4 backdrop-blur">
            <div className="relative mb-4 h-5">
              ...
              {/* Range input sits on top so the bar can be dragged */}
              <input
                type="range"
                min="0"
                max="100"
